*&---------------------------------------------------------------------*
* 모듈/서브모듈   : SD/SDC
* Program ID : ZR18A00030
* Desc       : Create Good Issue by Delivery
...
* do_gi Link  " 이거 헤더 테이블로 해도 상관 없을 듯.
DATA: gt_dogi TYPE TABLE OF zlikp18 WITH NON-UNIQUE SORTED KEY idx01
        COMPONENTS vbeln. " do_gi 연결 확인용. 즉 이미 생성 된건 지 확인(중복 방지)
...
*        COMPONENTS vbeln. " do의 아이템 테이블  (이거 없어도 될듯)
  ...
  " DATA: lt_lipsvv TYPE TABLE OF zlips18. " 이미 만들어진 gi 키값. ****
  " 이거 없이 lt_lipsv 로 다할 수 있을 것 같아요... 다른 프로그램에서도 가능할것 같은데.
  " 일단 여기서만 해보겠니다!**********
  ...
  REFRESH: gt_likp18, gt_lips18, gt_dogi.
*&--------------------------------*
...
*&--------------------------------*
" 이미 gi가 생성되지 않은 레코드를 lt_lipsv에서 삭제   ****
*&--------------------------------*
    DELETE lt_lipsv WHERE wadat_ist is INITIAL. " gi 생성 날짜 없으면 삭제

*&--------------------------------*
" 이미 gi가 생성된 do 아이템을 gt_dogi에 넣기(나중에 중복 확인용)  ****
*&--------------------------------*
*    SORT lt_lipsv BY vbeln. " 정렬 및 중복 제거   " 위에서 이미 정렬, 중복제거 해서 필요 없을듯.
*    DELETE ADJACENT DUPLICATES FROM lt_lipsv COMPARING vbeln.
    gt_dogi = lt_lipsv. " 넣기
...
FORM av2000_1_toolbar
  ...
  IF sy-tcode <> 'ZR18A00060'.
    ...
    ls_tool-function  = zle18_CRGI.  " gi 만들기(업데이트)
    ls_tool-icon      = icon_led_inactive.
    ls_tool-text      = TEXT-t01.   " 버튼 텍스트
    APPEND ls_tool TO po_object->mt_toolbar.
  ENDIF.
...
* 1개건만 진행
  ...
  LOOP AT pt_rows ASSIGNING FIELD-SYMBOL(<ls_rows>).  " 선택된 row
      ...
      IF sy-subrc = 0.
        CASE pv_ucomm.
          WHEN 'CRGI'.    " 아이템
            PERFORM zz_get_selrows_precheck USING <ls_likp> CHANGING pv_error.   " 이미 GI 가 생성 됐는지 확인하는 서브 루틴.
        ENDCASE.
      ENDIF.
...
FORM zz_get_selrows_precheck  USING    ps_likp LIKE LINE OF gt_likp18
  ...
  READ TABLE gt_dogi TRANSPORTING NO FIELDS   " 아까 만들어둔 이미 do_gi 링크가 생성된 변수 읽기.
...
FORM zz_set_sel_rows_ucomm  USING    pv_ucomm
  ...
  CALL METHOD zcl18_lec_auto_plan=>zz_get_gi_rtn  " gi 만들기
    IMPORTING
      es_return = ls_return
    CHANGING
      cs_do_h   = ps_likp.
